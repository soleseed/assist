...
<script type="text/html" id="statustmp">
		{{#	if(d.fangliuStatus == 0 || d.fangliuStatus == null){ }}
 ...
 // 对Date的扩展，将 Date 转化为指定格式的String
         ...
         index: 'lib/index' //主入口模块
    		...
    		//执行一个 table 实例
      		var tableIns = table.render({
    		     elem: '#result-table'
    		    ,height: 480
              ,url: '${pageContext.request.contextPath }/scheduleJob/searchByPost?statusParam=fangliuStatus' //数据接口
              ,page:true
              ,even:true
              ,cols: [[ //表头
                 /* {type:'checkbox'} */
            ...
            table.on('tool(result)', function(obj){ 
              ...
              if(obj.event === 'get'){
            	  $.ajax({
                      ...
                      //防溜/撤除防溜作业接收时分 （反馈类型）
                      data:{"id":data.id,"fangliuStatus": 1,"antiStartTime": new Date().format("yyyy-MM-dd hh:mm:ss"), 
                      ...
                      success:function(data,status){
	    					...
	    					//检查是否仍有其他未接收的任务
  							$.ajax({
  				                ...
  				                async:false,
  				                cache:false,
  				                url:"${pageContext.request.contextPath }/scheduleJob/searchNewJob?statusParam=fangliuStatus",
  				                dataType:"json",
  				                success:function(data,status){
  				                	if(data.count != 0){
  				                		//仍有新任务没接收,播放语音提醒
            	  ...
            	  layer.open({
            	        ...
            	        area: ['500px','400px'],
            	        title: '',
            	        content: '${pageContext.request.contextPath }/scheduleJob/addFangliu?isArrive='+ data.isArrive + '&jobId='+ data.id + '&track='+ data.trackName,
    ...
    function finish(){
    	//
         ...
         var antiTool = body.find('select[name="antiTool"]').val();
         var antiLocation = body.find('select[name="antiLocation"]').val();
    	$.ajax({
            type:"post",
            async:false,
            cache:false,
            url:"${pageContext.request.contextPath }/scheduleJob/updateStatus",
           //防溜/撤除防溜作业完成时分 （反馈类型）
            data:{"id":id,"fangliuStatus": 2,"antiFinishTime": new Date().format("yyyy-MM-dd hh:mm:ss"), 
          	  "jobType":9,"feedbackTime": new Date().format("yyyy-MM-dd hh:mm:ss"),
          	  "antiRunningCount":antiRunningCount, "antiRunningMethod":antiRunningMethod,
          	  "antiRunningMan":antiRunningMan,"antiTool": antiTool,"antiLocation": antiLocation},
            dataType:"json",
            success:function(data,status){
            	//父窗口
				 window.location.reload();
